...
4. Verify port is not already in use: ss -tlnp | grep 8069
...
```bash
# Test connectivity
psql -h lab-db1 -U odoo_user -d odoo_db -c '\conninfo'
...
1. Check resource usage: docker stats it-stack-odoo
...
| Log | Path |
|-----|------|
| Application | docker compose logs odoo |
